...
```mermaid
flowchart LR
    A[Baseline + Candidate\nPDF or HWPX] --> B[Format-safe Extractor]
...
| 경로 | 주 담당 | 변경 시 확인 대상 |
|---|---|---|
| `src/indexguard/extractors`, `detectors`, `decision.py`, `apps/api` | 기술 리드 | AI 리드의 fixture 기대값 |
| `apps/dashboard`, `README.md`, 배포 파일 | 제품·기획 리드 | API 계약 |
| `src/indexguard/llm`, `tests/fixtures`, `data`, 평가 스크립트 | AI·레드팀 리드 | 공통 finding enum |
