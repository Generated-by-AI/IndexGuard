...
IndexGuard는 **PDF·HWPX 문서의 변경을 규칙과 로컬 LLM으로 분석해 숫자·정책 왜곡, 숨겨진 콘텐츠, 간접 프롬프트 인젝션을 임베딩 전에 차단하는 RAG 보안 게이트**입니다.
...
HWPX는 한컴 후원 맥락을 보여주는 **대표 입력 형식(P0)** 입니다. PDF도 같은 분석 파이프라인으로 지원하지만, 레거시 바이너리 `.hwp`는 이번 MVP에서 다루지 않습니다.
